		...
		Task<List<ExpenseInstance>> task = new Task<List<ExpenseInstance>>() {
			...
			protected List<ExpenseInstance> call() throws Exception {
				...
				try (Session s = App.sf().openSession()) {
					list = s.createNamedQuery("getExpenseInstancesForCategory", ExpenseInstance.class).setParameter("tp", timePeriod.getID()).setParameter("cat", category.ID.get()).getResultList();
				}
				return list;
			}
		};
